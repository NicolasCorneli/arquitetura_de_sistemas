  ...
  try {
    ...
    if (!Array.isArray(products) || products.length === 0) {
      return res.status(400).json({
        error: "products é obrigatório e deve ter ao menos 1 item"
      });
    }
    ...
    // normaliza produtos + calcula totalAmount consultando product-service
    const normProducts = [];
    let totalAmount = 0;
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      // Busca o produto real no product-service
      let productData;
      try {
        // log opcional para debug local — comente se quiser
        // console.log("🔎 Fetching product:", `${PRODUCTS_BASE}/${pid}`);
        const resp = await axiosInstance.get(`${PRODUCTS_BASE}/${pid}`);
        productData = resp.data;
      } catch (err) {
        // se a chamada falhar (host inacessível ou 404), retorna 404
