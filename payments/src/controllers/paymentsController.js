...
const { sendNotification } = require("../rabbit"); // rabbit.js está em src/
...
async function processPayment(req, res) {
  try {
    ...
    // Atualiza pedido para PAID (sem aguardar)
    axiosInstance.patch(`${ORDERS_BASE}/${orderId}/status`, { status: "PAID" }).catch(() => {});

    // Enfileira notificação no RabbitMQ
    ...
    try {
      ...
      console.log(`📨 Notificação enviada para fila (cliente ${clientId}): ${message}`);
    } catch (e) {
      console.error("Erro ao enviar notificação via RabbitMQ:", e);
    }
...
async function createPayment(req, res) {
  try {
    ...
    // Amount vem automaticamente do pedido
